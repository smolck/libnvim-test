...
{
  renderer_updateMenu(ui->data);
}

static void busy_start(UI *ui)
{
  renderer_busyStart(ui->data);
}

static void busy_stop(UI *ui)
{
  renderer_busyStop(ui->data);
}

static void mouse_on(UI *ui)
{
  renderer_mouseOn(ui->data);
}

static void mouse_off(UI *ui)
{
  renderer_mouseOff(ui->data);
}
...
{
  renderer_bell(ui->data);
}

static void visual_bell(UI *ui)
{
  renderer_visualBell(ui->data);
}

static void grid_scroll(UI *ui,
    Integer g, Integer startrow,  // -V751
    Integer endrow, Integer startcol,
    Integer endcol, Integer rows,
    Integer cols
)
{
  renderer_gridScroll(ui->data, g, startrow, endrow, startcol, endcol, rows, cols);
}

static void default_colors_set(UI *ui, Integer rgb_fg, Integer rgb_bg, Integer rgb_sp,
                                   Integer cterm_fg, Integer cterm_bg)
{
  renderer_defaultColorsSet(ui->data, rgb_fg, rgb_bg, rgb_sp, cterm_fg, cterm_bg);
}

// Will this function run in reverse in the southern hemisphere?
static void flush(UI *ui)
{
  renderer_flush(ui->data);
}

static void suspend(UI *ui)
{
  renderer_suspend(ui->data);
}
...
static void raw_line(UI *ui,
    Integer g, Integer linerow,
    Integer startcol, Integer endcol,
    Integer clearcol, Integer clearattr,
    int flags, const schar_T *chunk, const sattr_T *attrs)
{
  renderer_rawLine(ui->data,
    g, linerow,
    startcol, endcol,
    clearcol, clearattr,
