...
void Renderer::initGlfwAndSkia() {
  ...
  std::unique_ptr<GLFWwindow, GLFWDestructor> win(glfwCreateWindow(
      INITIAL_WIDTH, INITIAL_HEIGHT, "Simple example", NULL, NULL));
  ...
  std::unique_ptr<SkSurface> surface(
      SkSurface::MakeFromBackendRenderTarget(
          this->m_context.get(), backendRenderTarget,
...
Renderer::Renderer() { this->initGlfwAndSkia(); }
...
void Renderer::gridScroll(int gridId, int startRow, int endRow, int startCol,
...
void Renderer::defaultColorsSet(int rgbFg, int rgbBg, int rgbSp, int ctermFg,
                                int ctermBg) {}
void Renderer::flush() {}
...
void Renderer::rawLine(int gridId, int lineRow, int startCol, int endCol,
                       int clearCol, int clearAttr, int flags,
